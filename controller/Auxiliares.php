...
class Auxiliares extends Controllers
{
    ...
    function auxiliares()
    {
        ...
        if ($sesion->getLogin()) {
            ...
            $data['tag_page'] = "Auxiliares -  Tienda Virtual ";
            $data['page_title'] = "Auxiliares - <small> Tienda Virtual </small>";
            $data['page_name'] = "auxiliares";
            
           // $data['ltabla'] = $ltabla;
            //llamado a la vista
            $this->views->getViews($this, "auxiliares", $data);
        } else {
            # code...
    ...
    // Lista para el combo box de tipo de tablas
   ...
   function desactivar($id){
        ...
        if ($data){
            header("location:" . base_url() . "/AuxiliaresSub");
    ...
    function activar($data){
        ...
        if ($data){
            header("location:" . base_url() . "/Auxiliares");
    ...
    function validaData($data)
    {
        ...
        if ($r > 0){
                array_push($errores, "Auxiliares ya existe");
         }
    ...
    function cambio($id)
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            //$data['data'] = $data;
            $data['tag_page'] = " Auxiliares";
            $data['page_title'] = "Auxiliares";
            $data['page_name'] = "Auxiliares";
            $data['page_name'] = "Auxiliares";
            $data['card_title'] = "Edición Auxiliares";

            //llamar a la vista que queremos ver

            $this->views->getViews($this, "AuxiliaresEditar", $data);
        }
    }
        ...
        $errores = array();
          ...
          if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            if (count($errores) == 0) {
                ...
                $data['tag_page'] = "AuxiliaresSub Alta";
                $data['page_title'] = "AuxiliaresSub ";
                $data['page_name'] = "AuxiliaresSub Alta";
                $data['card_title'] = "AuxiliaresSub Alta" ;
                $data['errores'] = $errores;
                //llamado a la vista 

                $this->views->getViews($this, "AuxiliaresAlta", $data);
            }
        } else {
   ...
   /**
    ...
    function listar()
    {
        ...
        for ($i = 0; $i < count($data); $i++) {
            ...
            // Identifico el estado de auxiliaress
            $camino = base_url();
            ...
            if ($data[$i]['condicion'] == 1) {
                ...
                $data[$i]['options'] = '<div class = "text-center">
                        <a href="' . base_url() . '/AuxiliaresSub/cambio/' . $data[$i]['idauxiliares'] . ' " title= "Editar"  " class="btn-outline-primary btn-sm btnAuxiliares"><i class = "fas fa-pencil-alt"></i></a> 
                        <a href="' . base_url() . '/AuxiliaresSub/desactivar/' . $data[$i]['idauxiliares'] . ' " title= "Eliminar"  " class="btn-outline-danger btn-sm btnAuxiliares"><i class = " fas fa-trash-alt""></i></a> 
                        <a href="' . base_url() . '/AuxiliaresSub/desactivar/' . $data[$i]['idauxiliares'] . ' " title= "Añadir"  " class="btn-outline-warning btn-sm btnAuxiliares"><i class = " fas fa-folder-plus""></i></a> 
                ...
                $data[$i]['condicion'] = '<span class="badge badge-danger">Inactivo</span>'; 
                 // Añadir Acciones al formulario
                $data[$i]['options'] = '<div class = "text-center">
                <a href="' . base_url() . '/auxiliares/cambio/' . $data[$i]['idauxiliares'] . ' " title= "Editar"  " class="btn-outline-primary btn-sm btnAuxiliares"><i class = "fas fa-pencil-alt"></i></a> 
                <a href="' . base_url() . '/auxiliares/activar/' . $data[$i]['idauxiliares'] . ' " title= "Activar"  " class="btn-outline-success btn-sm btnAuxiliares"><i class = "fas fa-undo"></i></a> 
                <a href="' . base_url() . '/AuxiliaresSub/desactivar/' . $data[$i]['idauxiliares'] . ' " title= "Añadir"  " class="btn-outline-warning btn-sm btnAuxiliares"><i class = " fas fa-folder-plus""></i></a> 
             </div>';
            }
