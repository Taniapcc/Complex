        ...
        for ($i = 0; $i < count($data); $i++) {
            ...
            if ($data[$i]['condicion'] == 1) {
                ...
                $data[$i]['options'] = '<div class = "text-center">
                        <a href="' . base_url() . '/Proveedor/cambio/' . $data[$i]['idusuario'] . ' " title= "Editar"  " class="btn-outline-primary btn-sm btnCategoria"><i class = "fas fa-pencil-alt"></i></a> 
                        <a href="' . base_url() . '/Proveedor/desactivar/' . $data[$i]['idusuario'] . ' " title= "Eliminar"  " class="btn-outline-danger btn-sm btnCategoria"><i class = " fas fa-trash-alt""></i></a> 
                ...
                $data[$i]['options'] = '<div class = "text-center">
                <a href="' . base_url() . '/Proveedor/cambio/' . $data[$i]['idusuario'] . ' " title= "Editar"  " class="btn-outline-primary btn-sm btnCategoria"><i class = "fas fa-pencil-alt"></i></a> 
                <a href="' . base_url() . '/Proveedor/activar/' . $data[$i]['idusuario'] . ' " title= "Activar"  " class="btn-outline-success btn-sm btnCategoria"><i class = "fas fa-undo"></i></a> 
    ...
    function alta()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            if (count($errores) == 0) {
                ...
                $data['tag_page'] = " Alta";
                $data['page_title'] = "Alta Proveedor";
                $data['page_name'] = "Alta Proveedor";
                $data['errores'] = $errores;
                //llamado a la vista 

                $this->views->getViews($this, "proveedorAlta", $data);
            }
        } else {

            $data['tag_page'] = "Proveedor Alta";
            $data['page_title'] = "Alta Proveedor";
            $data['page_name'] = "Alta Proveedor";
            //llamado a la vista  
    ...
    function cambio($id)
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            if (count($errores) == 0) {
                ...
                echo "Existe  Errores en el ingreso de información";
                dep($errores);
            ...
            $this->views->getViews($this, "proveedorModifica", $data);
        }
    }
}
