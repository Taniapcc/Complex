...
class AuxiliaresSub extends Controllers
{
    ...
    function auxiliaresSub()
    {
        ...
        if ($sesion->getLogin()) {
            # code...
                        ...
                        $data['tag_page'] = "AuxiliaresSub -  Tienda Virtual ";
            $data['page_title'] = "AuxiliaresSub - <small> Tienda Virtual </small>";
            $data['page_name'] = "auxiliaresSub";
            
           // $data['ltabla'] = $ltabla;
            //llamado a la vista
            $this->views->getViews($this, "auxiliaresSub", $data);
        } else {
            # code...
   ...
   function desactivar($id){
    ...
    function validaData($data)
    {
        ...
        if ($r > 0){
                array_push($errores, "AuxiliaresSub ya existe");
         }
    ...
    function cambio($id)
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            //$data['data'] = $data;
            $data['tag_page'] = " AuxiliaresSub";
            $data['page_title'] = "AuxiliaresSub";
            $data['page_name'] = "AuxiliaresSub";
            $data['page_name'] = "AuxiliaresSub";
            $data['card_title'] = "Edición AuxiliaresSub";
    ...
    function listar()
    {
        ...
        for ($i = 0; $i < count($data); $i++) {
            ...
            if ($data[$i]['condicion'] == 1) {
                ...
                $data[$i]['options'] = '<div class = "text-center">
                <a href="' . base_url() . '/auxiliaresSub/cambio/' . $data[$i]['idauxiliares'] . ' " title= "Editar"  " class="btn-outline-primary btn-sm btnAuxiliares"><i class = "fas fa-pencil-alt"></i></a> 
                <a href="' . base_url() . '/auxiliaresSub/activar/' . $data[$i]['idauxiliares'] . ' " title= "Activar"  " class="btn-outline-success btn-sm btnAuxiliares"><i class = "fas fa-undo"></i></a> 
                <a href="' . base_url() . '/AuxiliaresSub/desactivar/' . $data[$i]['idauxiliares'] . ' " title= "Añadir"  " class="btn-outline-warning btn-sm btnAuxiliares"><i class = " fas fa-folder-plus""></i></a> 
             </div>';
            }
