<?php
    class ProveedorModel extends Mysql
    {
       ...
       {
           $sql="SELECT * FROM v_proveedor where email = '$email'";
     ...
     function validaLogin($login){
        $sql = "SELECT * FROM v_proveedor WHERE tipousuario = 'Empleado' AND login ='".$login."'";
        $rows = $this->queryRows($sql);  
        return ($rows==0)?true:false;
      }  
	 ...
	 function listar()
     {
         $sql="SELECT idusuario,cedula,nombre,direccion,telefono,tipoUsuario,condicion FROM v_proveedor";
